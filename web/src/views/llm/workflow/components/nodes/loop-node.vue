    ...
    <div class="bg-white border border-gray-200 rounded-lg w-60 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div class="flex items-center justify-between px-3 py-2 border-b border-gray-100">
        <div class="flex items-center">
          <t-icon name="refresh" class="text-orange-600 mr-2" />
      ...
      <div class="px-3 py-2 flex flex-col gap-2">
        ...
        <div class="flex items-center justify-between">
          ...
          <span class="text-xs font-medium text-gray-700">{{ displayLoopType }}</span>
        </div>

        <!-- 循环条件/次数 -->
        <div class="flex items-center justify-between">
          <span class="text-xs text-gray-500">条件</span>
          <span class="text-xs font-medium text-gray-700 truncate max-w-32">{{ displayCondition }}</span>
        </div>

        <!-- 最大迭代次数 -->
        <div v-if="data.config?.loopType === 'for'" class="flex items-center justify-between">
          <span class="text-xs text-gray-500">次数</span>
          <span class="text-xs font-medium text-gray-700">{{ data.config?.maxIterations || 10 }}</span>
        </div>
        ...
        <div class="relative flex items-center justify-center py-1">
          ...
          <span class="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">循环体</span>
        </div>
        ...
        <div class="relative flex items-center justify-center py-1">
          ...
          <span class="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">结束</span>
        </div>
      </div>
    </div>
