    ...
    <div class="bg-white border border-gray-200 rounded-lg w-60 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div class="flex items-center justify-between px-3 py-2 border-b border-gray-100">
        <div class="flex items-center gap-2">
          <t-icon name="database" class="text-lg text-gray-700" />
      ...
      <div class="px-3 py-2 flex flex-col gap-2">
        ...
        <div class="flex items-center gap-1.5">
          ...
          <span class="text-xs text-gray-700">{{ displayOperationType }}</span>
        </div>

        <!-- 数据源 -->
        <div class="flex items-center gap-1.5">
          <span class="text-xs text-gray-400">数据源</span>
          <span class="text-xs text-gray-700 truncate max-w-32">{{ data.config?.dataSource || 'default' }}</span>
        </div>

        <!-- 表名 -->
        <div class="flex items-center gap-1.5">
          <span class="text-xs text-gray-400">表名</span>
          <span class="text-xs text-gray-700 truncate max-w-32">{{ data.config?.tableName || 'table' }}</span>
        </div>

        <!-- 条件数量 -->
        <div v-if="data.config?.operationType === 'select'" class="flex items-center gap-1.5">
          <span class="text-xs text-gray-400">条件</span>
          <span class="text-xs text-gray-700">{{ conditionCount }} 条</span>
        </div>

        <!-- 字段数量 -->
        <div v-if="data.config?.operationType === 'select'" class="flex items-center gap-1.5">
          <span class="text-xs text-gray-400">字段</span>
          <span class="text-xs text-gray-700">{{ fieldCount }} 个</span>
        </div>

        <!-- 输出 -->
        <div class="flex items-center gap-1.5">
          <span class="text-xs text-gray-400">输出</span>
          <span class="text-xs text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded">{{ data.config?.outputVariable || 'output' }}</span>
        </div>
      </div>
    </div>
