...
const configForm = computed(() => {
  switch (nodeData.value.type) {
    ...
    case 'output':
      return [
        { key: 'outputType', label: '输出类型', type: 'select', options: [
          { value: 'text', label: '文本' },
          { value: 'json', label: 'JSON' },
          { value: 'file', label: '文件' },
        ]},
    ...
    case 'llm':
      return [
        { key: 'model', label: '模型名称', type: 'select', options: [
    ...
    case 'prompt':
      return [
        { key: 'template', label: '提示词模板', type: 'textarea' },
        { key: 'variables', label: '变量列表', type: 'tags' },
      ];
    ...
    case 'loop':
      return [
        { key: 'loopType', label: '循环类型', type: 'select', options: [
    ...
    case 'database':
      return [
        { key: 'connectionString', label: '连接字符串', type: 'input' },
        { key: 'query', label: 'SQL查询', type: 'textarea' },
        { key: 'parameters', label: '查询参数', type: 'json' },
      ];
    
    case 'transform':
      return [
        { key: 'transformType', label: '转换类型', type: 'select', options: [
          { value: 'json', label: 'JSON转换' },
          { value: 'text', label: '文本处理' },
          { value: 'format', label: '格式化' },
          { value: 'extract', label: '数据提取' },
        ]},
        { key: 'script', label: '转换脚本', type: 'textarea' },
      ];
  ...
  <div class="node-config p-4 space-y-6">
    ...
    <div>
      ...
      <div class="space-y-4">
        <div>
          ...
          <t-input
            v-model:value="nodeData.data.label"
            placeholder="请输入节点名称"
            @blur="updateNode"
          />
        </div>
      ...
      <div class="space-y-4">
        <div v-for="field in configForm" :key="field.key">
          ...
          <t-textarea
            ...
            :model-value="typeof getConfigValue(field.key) === 'object' 
    ...
    <div>
      ...
      <div class="space-y-3">
        ...
        <div v-if="nodeData.data.inputs.length === 0" class="text-center text-gray-500 py-4">
          暂无输入端口
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <div class="space-y-3">
        ...
        <div v-if="nodeData.data.outputs.length === 0" class="text-center text-gray-500 py-4">
          暂无输出端口
        </div>
      </div>
    </div>
