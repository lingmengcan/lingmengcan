        ...
        <div class="space-y-2">
          ...
          >
            <div class="flex items-center justify-between">
              <span class="text-xs font-medium text-gray-600">分支 {{ index + 1 }}</span>
            ...
            <div class="grid grid-cols-2 gap-2">
              <t-form-item label="超时(秒)" class="compact-form-item">
                <t-input-number v-model="branch.timeout" :min="1" :max="300" size="small" @change="updateConfig" />
              </t-form-item>
              <t-form-item label="重试" class="compact-form-item">
        ...
        <div class="space-y-3">
          ...
          <t-form-item label="错误处理" class="compact-form-item">
            <t-select v-model="localConfig.errorHandling" size="small" @change="updateConfig">
              ...
              <t-option value="ignore" label="忽略错误" />
            </t-select>
          </t-form-item>
...
<script setup lang="ts">
  ...
  const createDefaultBranches = (count: number): Branch[] => {
  ...
  const localConfig = reactive({
    ...
    branches: (props.node?.data?.config?.branches as Branch[]) || createDefaultBranches(2),
    ...
    (newNode, oldNode) => {
      ...
      if (newNode && newNode.id !== oldNode?.id) {
        Object.assign(localConfig, {
          ...
          branches: newNode.data?.config?.branches || createDefaultBranches(2),
