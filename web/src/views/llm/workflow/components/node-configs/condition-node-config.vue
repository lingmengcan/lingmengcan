<template>
  <div class="space-y-4 p-3">
    ...
    <div class="border border-gray-200 rounded-lg overflow-hidden">
      <div class="bg-gray-50 px-4 py-2 border-b border-gray-200">
        <div class="flex items-center gap-2">
          <span class="text-lg font-medium text-gray-700">IF</span>
          <span class="text-sm text-gray-500">CASE1</span>
        </div>
      </div>
    ...
    >
      <div class="bg-gray-50 px-4 py-2 border-b border-gray-200">
        <div class="flex items-center gap-2">
          <span class="text-lg font-medium text-gray-700">ELIF</span>
    ...
    <div class="bg-gray-50 rounded-lg p-4">
      <div class="mb-2">
        <span class="text-lg font-medium text-gray-700">ELSE</span>
      </div>
...
<script setup lang="ts">
  ...
  const localConfig = reactive({
    ...
    ifConditions: props.node?.data?.config?.ifConditions || [
      { variable: '', operator: '', value: '', logic: 'AND' as 'AND' | 'OR' },
    ],
    ...
    (newNode, oldNode) => {
      ...
      if (newNode && newNode.id !== oldNode?.id) {
        ...
        Object.assign(localConfig, {
          ...
          elifCases: newNode.data?.config?.elifCases || ([] as ElifCase[]),
        });
      }
    },
    { deep: true },
  ...
  const addElifCase = () => {
    localConfig.elifCases.push({
      conditions: [{ variable: '', operator: '', value: '', logic: 'AND' }],
    });
    updateConfig();
  };
  ...
  const removeCase = (type: 'if' | 'elif', caseIndex?: number) => {
    if (type === 'if') {
      ...
      if (localConfig.elifCases.length > 0) {
        ...
        // 如果没有 ELIF 分支，重置 IF 条件为默认值
        localConfig.ifConditions = [{ variable: '', operator: '', value: '', logic: 'AND' }];
      }
      updateConfig();
    } else if (type === 'elif' && typeof caseIndex === 'number') {
